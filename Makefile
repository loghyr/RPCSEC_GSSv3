...
txt: draft-ietf-nfsv4-rpcsecgssv3-$(VERS).txt

html: draft-ietf-nfsv4-rpcsecgssv3-$(VERS).html

nr: draft-ietf-nfsv4-rpcsecgssv3-$(VERS).nr

xml: draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml

clobber:
	$(RM) draft-ietf-nfsv4-rpcsecgssv3-$(VERS).txt \
		draft-ietf-nfsv4-rpcsecgssv3-$(VERS).html \
		draft-ietf-nfsv4-rpcsecgssv3-$(VERS).nr
...
clean:
	rm -f $(AUTOGEN)
	rm -rf autogen
	rm -f draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml
	rm -rf draft-$(VERS)
...
draft-ietf-nfsv4-rpcsecgssv3-$(VERS).txt: draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml
	rm -f $@ draft-tmp.txt
	sh xml2rfc_wrapper.sh draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml draft-tmp.txt
	mv draft-tmp.txt $@

draft-ietf-nfsv4-rpcsecgssv3-$(VERS).html: draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml
	rm -f $@ draft-tmp.html
	sh xml2rfc_wrapper.sh draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml draft-tmp.html
	mv draft-tmp.html $@

draft-ietf-nfsv4-rpcsecgssv3-$(VERS).nr: draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml
	rm -f $@ draft-tmp.nr
	sh xml2rfc_wrapper.sh draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml $@.tmp
	mv draft-tmp.nr $@
...
draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml: draft-tmp.xml $(IDCONTENTS) $(AUTOGEN)
		rm -f $@
		cp draft-tmp.xml $@

genhtml: Makefile gendraft html txt draft-$(VERS).tar
	./gendraft draft-$(PREVVERS) \
		draft-ietf-nfsv4-rpcsecgssv3-$(PREVVERS).txt \
		draft-$(VERS) \
		draft-ietf-nfsv4-rpcsecgssv3-$(VERS).txt \
		draft-ietf-nfsv4-rpcsecgssv3-$(VERS).html \
		draft-ietf-nfsv4-rpcsecgssv3-dot-x-04.txt \
		draft-ietf-nfsv4-rpcsecgssv3-dot-x-05.txt \
		draft-$(VERS).tar.gz
...
DRAFTFILES = \
	draft-ietf-nfsv4-rpcsecgssv3-$(VERS).txt \
	draft-ietf-nfsv4-rpcsecgssv3-$(VERS).html \
	draft-ietf-nfsv4-rpcsecgssv3-$(VERS).xml
